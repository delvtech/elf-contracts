...
contract TestElf {
  ...
  function testDepositWithdraw() public {
    ...
    Assert.equal(elf.balanceOf(address(this)),1,"Shares are wrong after deposit");

    elf.withdraw(1);
    Assert.equal(elf.balance(),0 ether,"Balance is wrong after withdraw");
    Assert.equal(elf.balanceOf(address(this)),0,"Shares are wrong after withdraw");

    (success, ) = address(elf).call{gas: 200317, value: 1 ether}(abi.encodeWithSignature("deposit()"));
    require(success, "Failed to transfer the funds, aborting.");
    Assert.equal(elf.balance(),1 ether,"Balance is wrong after 2nd deposit");
    Assert.equal(elf.balanceOf(address(this)),1,"Shares are wrong after 2nd deposit");
