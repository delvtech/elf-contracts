...
contract WrappedFuturesPrincipalFactory is Authorizable {
    ...
    event WrappedFuturesPrincipalCreated(address indexed _baseToken, address indexed _owner);
    ...
    function create(address _baseToken, address _owner) external onlyOwner returns(address) {
        // Validate the given params
        _zeroAddressCheck(_owner);
        _zeroAddressCheck(_baseToken);
        address wfPrincipal = address(new WrappedFuturesPrincipal(_baseToken, _owner));
